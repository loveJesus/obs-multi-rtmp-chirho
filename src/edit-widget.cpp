...
class EditOutputWidgetImpl: public EditOutputWidget
{
    ...
    public:
        ...
        void UpdateProperties(obs_properties* props, obs_data* settings) {
            blog(LOG_INFO, "obs-multi-rtmp: UpdateProperties - Start");
            try {
                ResetWid();
                layout_->addWidget(wid_ = createPropertyWidget(props, settings_ = settings, this), 0, 0, 1, 1);
                obs_data_release(settings);                
                blog(LOG_INFO, "obs-multi-rtmp: UpdateProperties - Complete");
            } catch (const std::exception& e) {
                blog(LOG_ERROR, "obs-multi-rtmp: Exception in UpdateProperties: %s", e.what());
            } catch (...) {
                blog(LOG_ERROR, "obs-multi-rtmp: Unknown exception in UpdateProperties");
            }
    ...
    QTabWidget* CreateOutputSettingsWidget(QWidget* parent) {
        blog(LOG_INFO, "obs-multi-rtmp: CreateOutputSettingsWidget - Start");
        
        try {
            auto tab = new QTabWidget(parent);
            blog(LOG_INFO, "obs-multi-rtmp: Created QTabWidget");

            // service
            {
                blog(LOG_INFO, "obs-multi-rtmp: Creating service tab");
                serviceSettings_ = new PropertiesWidget(tab);
                blog(LOG_INFO, "obs-multi-rtmp: Created serviceSettings_");
                updateServiceTab();
                blog(LOG_INFO, "obs-multi-rtmp: Updated service tab");
                if (serviceSettings_) {
                    tab->addTab(serviceSettings_, obs_module_text("Tab.Service"));
                    blog(LOG_INFO, "obs-multi-rtmp: Added service tab");
                } else {
                    blog(LOG_ERROR, "obs-multi-rtmp: serviceSettings_ is null after updateServiceTab");
                }
            }

            // output
            {
                blog(LOG_INFO, "obs-multi-rtmp: Creating output tab");
                outputSettings_ = new PropertiesWidget(tab);
                blog(LOG_INFO, "obs-multi-rtmp: Created outputSettings_");
                updateOutputTab();
                blog(LOG_INFO, "obs-multi-rtmp: Updated output tab");
                tab->addTab(outputSettings_, obs_module_text("Tab.Output"));
                blog(LOG_INFO, "obs-multi-rtmp: Added output tab");
            }

            QObject::connect(tab, &QTabWidget::currentChanged, [tab](int index) {
                blog(LOG_INFO, "obs-multi-rtmp: Tab changed to index %d", index);
            ...
            blog(LOG_INFO, "obs-multi-rtmp: CreateOutputSettingsWidget - Complete");
            return tab;
        } catch (const std::exception& e) {
            blog(LOG_ERROR, "obs-multi-rtmp: Exception in CreateOutputSettingsWidget: %s", e.what());
        } catch (...) {
            blog(LOG_ERROR, "obs-multi-rtmp: Unknown exception in CreateOutputSettingsWidget");
        }
        
        blog(LOG_ERROR, "obs-multi-rtmp: CreateOutputSettingsWidget failed");
        return nullptr;
    }

    void updateServiceTab()
    {
        blog(LOG_INFO, "obs-multi-rtmp: updateServiceTab - Start");
        try {
            auto protocol_info = GetProtocolInfos()->GetInfo(config_->protocol.c_str());
            blog(LOG_INFO, "obs-multi-rtmp: Got protocol info");
            ...
            if (!protocol_info) {
                blog(LOG_ERROR, "obs-multi-rtmp: Failed to get valid protocol info");
                return;
            }

            blog(LOG_INFO, "obs-multi-rtmp: Protocol: %s, Service ID: %s", protocol_info->protocol, protocol_info->serviceId);

            blog(LOG_INFO, "obs-multi-rtmp: Creating service JSON");
            std::string json_str;
            if (config_->serviceParam.is_null()) {
                json_str = "{}";  // Use an empty object if serviceParam is null
                blog(LOG_WARNING, "obs-multi-rtmp: serviceParam is null, using empty object");
            } else {
                json_str = config_->serviceParam.dump();
            }
            blog(LOG_INFO, "obs-multi-rtmp: Service JSON: %s", json_str.c_str());

            blog(LOG_INFO, "obs-multi-rtmp: Creating obs_data from JSON");
            obs_data_t* settings = obs_data_create_from_json(json_str.c_str());
            if (!settings) {
                blog(LOG_ERROR, "obs-multi-rtmp: Failed to create obs_data from JSON");
                return;
            }

            blog(LOG_INFO, "obs-multi-rtmp: Creating service");
            auto service = obs_service_create(protocol_info->serviceId, ("tmp_service_" + targetid_).c_str(), settings, nullptr);
            obs_data_release(settings);
            
            if (!service) {
                blog(LOG_ERROR, "obs-multi-rtmp: Failed to create service");
                return;
            }
            
            blog(LOG_INFO, "obs-multi-rtmp: Getting service properties");
            auto properties = obs_service_properties(service);
            
            blog(LOG_INFO, "obs-multi-rtmp: Getting service settings");
            auto service_settings = obs_service_get_settings(service);
            
            blog(LOG_INFO, "obs-multi-rtmp: Updating serviceSettings_");
            serviceSettings_->UpdateProperties(properties, service_settings);
            
            blog(LOG_INFO, "obs-multi-rtmp: Releasing service");
            obs_service_release(service);
            
            blog(LOG_INFO, "obs-multi-rtmp: updateServiceTab - Complete");
        } catch (const std::exception& e) {
            blog(LOG_ERROR, "obs-multi-rtmp: Exception in updateServiceTab: %s", e.what());
        } catch (...) {
            blog(LOG_ERROR, "obs-multi-rtmp: Unknown exception in updateServiceTab");
        }
    }
...
public:
    ...
    , targetid_(targetid)
    {
        blog(LOG_INFO, "obs-multi-rtmp: EditOutputWidgetImpl constructor - Start");

        try {
            auto& global = GlobalMultiOutputConfig();
            config_ = FindById(global.targets, targetid_);
            if (config_ == nullptr) {
                blog(LOG_ERROR, "obs-multi-rtmp: Config not found for target ID: %s", targetid.c_str());
                return;
            }
            config_ = std::make_shared<OutputTargetConfig>(*config_);

            // Initialize protocol and serviceParam if not set
            if (config_->protocol.empty()) {
                config_->protocol = "RTMP";  // Set default protocol
                blog(LOG_INFO, "obs-multi-rtmp: Setting default protocol: RTMP");
            }
            if (config_->serviceParam.is_null()) {
                config_->serviceParam = nlohmann::json::object();  // Initialize as empty object
                blog(LOG_INFO, "obs-multi-rtmp: Initializing empty serviceParam");
            }

            blog(LOG_INFO, "obs-multi-rtmp: Setting window title");
            setWindowTitle(obs_module_text("StreamingSettings"));

            blog(LOG_INFO, "obs-multi-rtmp: Creating main layout");
            auto mainLayout = new QVBoxLayout(this);

            blog(LOG_INFO, "obs-multi-rtmp: Creating scroll area");
            ...
            blog(LOG_INFO, "obs-multi-rtmp: Creating container widget");
            container_ = new QWidget(scroll_);
            container_->setSizePolicy(QSizePolicy::Policy::Expanding, QSizePolicy::Policy::Expanding);

            blog(LOG_INFO, "obs-multi-rtmp: Creating container layout");
            auto containerLayout = new QVBoxLayout(container_);


            // Add your widgets to the containerLayout here
            blog(LOG_INFO, "obs-multi-rtmp: Adding streaming name and protocol widgets");
...

            {
                ...
                containerLayout->addLayout(sublayout);
            }

            blog(LOG_INFO, "obs-multi-rtmp: Creating output settings widget");
            ...
            blog(LOG_INFO, "obs-multi-rtmp: Creating output settings widget");
            {
                ...
                if (w) {
                    ...
                    blog(LOG_INFO, "obs-multi-rtmp: Output settings widget added to layout");
                } else {
                    blog(LOG_ERROR, "obs-multi-rtmp: Failed to create output settings widget");
                }
            }
            ++currow;
            blog(LOG_INFO, "obs-multi-rtmp: Adding video, audio, and other settings");

            {
                auto sub_grid = new QGridLayout();
                sub_grid->setColumnStretch(0, 1);
                sub_grid->setColumnStretch(1, 0);
                            blog(LOG_INFO, "obs-multi-rtmp: Adding video settings");

                {
                    ...
                    blog(LOG_INFO, "obs-multi-rtmp: Adding audio settings");
                  ...
                  blog(LOG_INFO, "obs-multi-rtmp: Adding other settings");
                ...
                containerLayout->addLayout(sub_grid);
            }
            blog(LOG_INFO, "obs-multi-rtmp: Adding OK button");

            {
                ...
                containerLayout->addWidget(okbtn);
            }

            containerLayout->setSizeConstraint(QLayout::SetFixedSize);
            blog(LOG_INFO, "obs-multi-rtmp: Setting up scroll area");

            scroll_->setWidget(container_);
            scroll_->setWidgetResizable(true);

            mainLayout->addWidget(scroll_);
            blog(LOG_INFO, "obs-multi-rtmp: Loading protocols");
            LoadProtocols();
            blog(LOG_INFO, "obs-multi-rtmp: Loading FPS denumerator");
            LoadFPSDenumerator();
            blog(LOG_INFO, "obs-multi-rtmp: Loading encoders");
            LoadEncoders();
            blog(LOG_INFO, "obs-multi-rtmp: Loading scenes");
            LoadScenes();

            blog(LOG_INFO, "obs-multi-rtmp: Loading config");
            LoadConfig();
            blog(LOG_INFO, "obs-multi-rtmp: Connecting widget signals");
            ConnectWidgetSignals();
            blog(LOG_INFO, "obs-multi-rtmp: Updating UI");
            UpdateUI();

            blog(LOG_INFO, "obs-multi-rtmp: Setting up event filter");

            // Event filter for resizing (unchanged)
            ...
            blog(LOG_INFO, "obs-multi-rtmp: EditOutputWidgetImpl constructor - Complete");
        } catch (const std::exception& e) {
            blog(LOG_ERROR, "obs-multi-rtmp: Exception in EditOutputWidgetImpl constructor: %s", e.what());
        } catch (...) {
            blog(LOG_ERROR, "obs-multi-rtmp: Unknown exception in EditOutputWidgetImpl constructor");
        }
    }
...
EditOutputWidget* createEditOutputWidget(const std::string& targetid, QWidget* parent) {
    blog(LOG_INFO, "obs-multi-rtmp: Creating EditOutputWidget - Start");
    try {
        auto widget = new EditOutputWidgetImpl(targetid, parent);
        blog(LOG_INFO, "obs-multi-rtmp: EditOutputWidget created successfully");
        return widget;
    } catch (const std::exception& e) {
        blog(LOG_ERROR, "obs-multi-rtmp: Exception in createEditOutputWidget: %s", e.what());
    } catch (...) {
        blog(LOG_ERROR, "obs-multi-rtmp: Unknown exception in createEditOutputWidget");
    }
    blog(LOG_ERROR, "obs-multi-rtmp: Failed to create EditOutputWidget");
    return nullptr;
}
